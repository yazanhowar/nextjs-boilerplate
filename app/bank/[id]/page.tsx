  ...
  return (
    <div className="min-h-screen bg-[var(--cf-bg)] text-[var(--cf-ink)]">
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        {/* OVERVIEW */}
        {activeTab === 'overview' && (
          <div className="space-y-6">{bankId === 2 && (<div style={{ background: 'var(--cf-surface, #fff)', border: '1px solid var(--cf-line, #e5eaf2)', borderRadius: 12, padding: 16 }}><div style={{ fontSize: 13, fontWeight: 700, color: 'var(--cf-ink, #0f2a4a)', marginBottom: 2 }}>International Footprint — Share of Group Deposits</div><div style={{ fontSize: 11, color: 'var(--cf-ink3, #7d8ea3)', marginBottom: 10 }}>Housing Bank operates across 6 markets · FY2025</div>{(function () { var geo: any = [{ n: 'Jordan', lon: 36, lat: 31.5, d: 82.5 }, { n: 'Palestine', lon: 34.5, lat: 32, d: 7.4 }, { n: 'Algeria', lon: 3, lat: 28, d: 4.1 }, { n: 'Britain', lon: -2, lat: 54, d: 3.0 }, { n: 'Bahrain', lon: 50.5, lat: 26, d: 1.8 }, { n: 'Syria', lon: 38.5, lat: 35.5, d: 1.3 }]; var W = 640, H = 320, mnLon = -16, mxLon = 56, mnLat = 16, mxLat = 60; var px = function (lo: number) { return ((lo - mnLon) / (mxLon - mnLon)) * W }; var py = function (la: number) { return ((mxLat - la) / (mxLat - mnLat)) * H }; var cols = ['#0f2a4a', '#2e6da4', '#3f8fc4', '#5aa0cf', '#8bbcde', '#c9a227']; return (<div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}><svg viewBox={'0 0 ' + W + ' ' + H} style={{ flex: '1 1 360px', minWidth: 280, height: 'auto', background: 'var(--cf-surface2, #f4f7fb)', borderRadius: 10 }}>{[0.2, 0.4, 0.6, 0.8].map(function (f: number, i: number) { return <line key={'v' + i} x1={f * W} y1={0} x2={f * W} y2={H} stroke='var(--cf-line, #e5eaf2)' strokeWidth={0.5} /> })}{[0.25, 0.5, 0.75].map(function (f: number, i: number) { return <line key={'h' + i} x1={0} y1={f * H} x2={W} y2={f * H} stroke='var(--cf-line, #e5eaf2)' strokeWidth={0.5} /> })}{geo.map(function (c: any, i: number) { var x = px(c.lon), y = py(c.lat); var r = 8 + Math.sqrt(c.d) * 2.6; return (<g key={i}><circle cx={x} cy={y} r={r} fill={cols[i]} fillOpacity={0.85} stroke='#fff' strokeWidth={1.5} />{r > 16 ? <text x={x} y={y + 4} textAnchor='middle' style={{ fontSize: 11, fontWeight: 700, fill: '#fff' }}>{c.d + '%'}</text> : null}</g>) })}</svg><div style={{ flex: '0 0 auto', display: 'flex', flexDirection: 'column', gap: 7 }}>{geo.map(function (c: any, i: number) { return (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}><span style={{ width: 11, height: 11, borderRadius: 3, background: cols[i], display: 'inline-block' }} /><span style={{ color: 'var(--cf-ink2, #3d4f66)', minWidth: 68 }}>{c.n}</span><span style={{ fontWeight: 700, color: 'var(--cf-ink, #0f2a4a)' }}>{c.d}%</span></div>) })}</div></div>); })()}<div style={{ fontSize: 10, color: 'var(--cf-ink3)', marginTop: 8 }}>Source: Housing Bank 2025 environment analysis (p26) · deposits by country</div></div>)}
