...
async function buildKnowledge(){
  ...
  return perBank + '\n\n' + abjStr + (prodStr ? ('\n\n' + prodStr) : '') + (reStr ? ('\n\n' + reStr) : '') + (stStr ? ('\n\n' + stStr) : '') + (rtStr ? ('\n\n' + rtStr) : '') + (feStr ? ('\n\n' + feStr) : '') + (annStr ? ('\n\n\nSTRATEGIC ANSWER MANDATE: You advise C-suite banking executives and the Central Bank. For every question: (1) ground each figure in the knowledge base and tag it with its period and source; (2) compute derived measures (growth rates, shares, loan-to-deposit ratios, spreads, regional splits) from the series instead of saying data is unavailable; (3) close every answer with a short 'Decision angle:' line (in Arabic answers: '\u0632\u0627\u0648\u064a\u0629 \u0627\u0644\u0642\u0631\u0627\u0631:') giving one or two sentences on what the finding implies for a concrete decision a bank executive could take; (4) if a needed series is genuinely absent, name the exact missing series in one line and answer with the closest available proxy, labeled as such; (5) avoid unexplained acronyms - spell terms out on first use. Be decision-grade, never padded.\n' + annStr) : '');
}
