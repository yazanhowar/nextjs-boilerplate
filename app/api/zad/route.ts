...
async function buildKnowledge(){
  ...
  var prodRes = await sb.from('bank_products').select('bank_id,category,sub_category,product_name_en,is_islamic,sharia_structure').eq('is_active', true).order('bank_id');
  ...
  var pByBank = {};
  prods.forEach(function(p){ var id = p.bank_id; (pByBank[id] = pByBank[id] || []).push(p.product_name_en + ' [' + (p.sub_category || p.category || '') + (p.is_islamic ? ' | islamic' + (p.sharia_structure ? ':' + p.sharia_structure : '') : '') + ']'); });
  var prodLines = [];
...
export async function POST(req){
  ...
  var __refBlock='';
    ...
    var ares = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model:'claude-opus-4-6', max_tokens:4096, system: (system + " [TRADE FINANCE SCOPE] You are also the Trade Finance Encyclopedia assistant for convo.finance. Trade finance and trade-law topics are fully in scope and must be answered, never refused as outside Jordanian banking. This includes documentary credits and letters of credit, demand guarantees and standby letters of credit, documentary collections, Incoterms 2020, all ICC rules (UCP 600, URDG 758, URC 522, ISP98, ISBP 821, eUCP, eURC, URR 725, URBPO, and URDTT the Uniform Rules for Digital Trade Transactions), SWIFT and ISO 20022 messaging, the UNCITRAL Model Law on Electronic Transferable Records (MLETR) and the UN Convention on Independent Guarantees and Standby Letters of Credit, FATF and trade-based money laundering, the Wolfsberg and BAFT trade finance principles, the Basel treatment of trade finance, UN Security Council sanctions, and CBJ and Jordanian trade regulation. Answer strictly from the reference entries provided below and the platform data you hold, applying the same discipline you use for the banking sector: do not state any ICC publication number, effective or adoption date, article number, rule or article count, or figure unless it appears in the reference material below. When a precise detail is not in your reference, give the substantive general answer and say plainly that the exact date or figure is not confirmed in your sources, rather than guessing or supplying one from memory. Match the user's language (Arabic or English). " + __refBlock + __tfBlock), messages: convo, stream: wantStream })
    });
