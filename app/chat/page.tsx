...
function extractCharts(text: string): { text: string; charts: any[] } {
  ...
  while (safety++ < 10) {
    ...
    if (si === -1) break
    const js = remaining[si + OPEN.length] === '\n' ? si + OPEN.length + 1 : si + OPEN.length
    const ci = remaining.indexOf(CLOSE, js)
    if (ci === -1) break
    try {
      const chart = JSON.parse(remaining.slice(js, ci).trim())
      charts.push(chart)
    } catch {}
    remaining = (remaining.slice(0, si) + remaining.slice(ci + CLOSE.length)).trim()
  }
...
function ChatContent() {
  ...
  async function send(content: string) {
    ...
    try {
      ...
      const { text, charts } = extractCharts(full)
      setMessages(prev => { const u = [...prev]; u[u.length-1] = { role:'assistant', content:text, charts: charts.length ? charts : undefined }; return u })
    } catch {
