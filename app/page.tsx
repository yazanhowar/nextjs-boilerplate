...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [curr, prev] = await Promise.all([
        ...
        sb.from('bank_financials').select('*').eq('fiscal_year', prevYear),
      ])
      if (curr.data && curr.data.length > 0) {
        ...
        const fallback = await sb.from('bank_financials').select('*').eq('fiscal_year', 2024)
        if (fallback.data) {
          const m: Record<number,any> = {}
          fallback.data.forEach((r: any) => { m[r.bank_id] = r })
          setFinancials(m)
          setDataYear(2024)
        }
      }
  ...
  const grandAssets = jodBanks.reduce((s,b) => s + (financials[b.id]?.total_assets ?? 0), 0)
    + (toJOD(financials[ARAB_BANK_ID]?.total_assets, ARAB_BANK_ID) ?? 0)
  const grandProfit = jodBanks.reduce((s,b) => s + (financials[b.id]?.net_profit ?? 0), 0)
    + (toJOD(financials[ARAB_BANK_ID]?.net_profit, ARAB_BANK_ID) ?? 0)
  const roeAvg = BANKS.map(b => financials[b.id]?.roe).filter(Boolean) as number[]
  const avgROE = roeAvg.length ? roeAvg.reduce((a,b) => a+b,0)/roeAvg.length : null
  const carAvg = BANKS.map(b => financials[b.id]?.car).filter(Boolean) as number[]
  const avgCAR = carAvg.length ? carAvg.reduce((a,b) => a+b,0)/carAvg.length : null
    ...
    <div style={{ minHeight: '100vh', backgroundColor: t.bg, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: t.text }}>
      ...
      <header style={{ backgroundColor: dark ? 'rgba(10,22,40,0.9)' : 'rgba(242,244,247,0.9)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: `1px solid ${t.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={toggleTheme} title={dark ? 'Light mode' : 'Dark mode'} style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${t.border}`, background: t.surface, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {dark ? '☀️' : '🌙'}
            </button>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {[2025, 2024, 2023].map(yr => (
                <button key={yr} onClick={() => setSelectedYear(yr)} style={{
                  ...
                  backgroundColor: selectedYear === yr ? t.accent : 'transparent',
                  color: selectedYear === yr ? '#fff' : t.textSub,
                  border: `1px solid ${selectedYear === yr ? t.accent : t.border}`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
          {filtered.map(bank => {
            ...
            return (
              <BankCard key={bank.id} bank={bank} fin={fin} delta={delta} loading={loading}
                dark={dark} t={t} dataYear={dataYear} hovered={hovered === bank.id}
                onMouseEnter={() => setHovered(bank.id)} onMouseLeave={() => setHovered(null)}
...
function BankCard({ bank, fin, delta, loading, dark, t, dataYear, hovered, onMouseEnter, onMouseLeave, onClick }: any) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} style={{
      ...
      borderRadius: 14, padding: 20, cursor: 'pointer', transition: 'all 0.15s ease',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ backgroundColor: hovered ? t.accent : (dark ? '#132240' : '#EEF2F7'), color: hovered ? '#fff' : t.textSub, borderRadius: 8, padding: '5px 12px', fontSize: 12, fontWeight: 500, transition: 'all 0.15s', whiteSpace: 'nowrap', marginLeft: 8 }}>
          Ask AI
        </span>
      </div>
    </div>
