  ...
  return (
    <div style={{ background: 'var(--cf-surface, #fff)', border: '1px solid var(--cf-line, #e5eaf2)', borderRadius: 12, padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--cf-ink, #0f2a4a)' }}>International Footprint</div>
      ...
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ flex: '2 1 440px', minWidth: 300, direction: 'ltr' }}>
          {err ? placeholder('Map data unavailable') : (!features || !path) ? placeholder('Loading map…') : (
            <svg viewBox={'0 0 ' + W + ' ' + H} width="100%" style={{ display: 'block' }} role="img" aria-label="Housing Bank Group geographic footprint">
        ...
        <div style={{ flex: '1 1 210px', minWidth: 196, display: 'flex', flexDirection: 'column', gap: 9 }}>
          {ranked.map((mk) => {
            const val = mk[metric];
            const w = Math.max((val / maxShare) * 100, 2);
            return (
              <div key={mk.id} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                <span style={{ flex: '1 1 auto', height: 6, background: '#eef2f7', borderRadius: 4, overflow: 'hidden' }}>
