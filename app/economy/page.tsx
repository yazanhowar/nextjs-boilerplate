      ...
      {segs.map(function (s: any, i: number) {
        ...
        return (
          <g key={'l' + i}>
            ...
            <text x={W - 12} y={ly} textAnchor='end' style={{ fontSize: 11, fontWeight: 600, fill: 'var(--cf-ink)' }}>{(s.v / 1000).toFixed(2)}<tspan style={{ fill: 'var(--cf-ink3, #7d8ea3)', fontWeight: 400 }}>{'  ' + s.pct.toFixed(1) + '%'}</tspan></text>
          </g>
        )
      })}
    </svg>
...
export default function EconomyPage() {
  ...
  var D = useMemo(function () {
    ...
    return {
      ...
      eShare: ep('Digital payment transactions'), eUsers: ep('Instant payment users'), eVal: ep('Digital payment value'), eAtm: ep('Mobile wallet users')
    }
  }, [rows, lang])
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(330px, 1fr))', gap: 12, marginTop: 14 }}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 6 }}>
              {[{ l: isAr ? 'المعاملات الرقمية' : 'Transactions', d: D.eShare, f: 1, suf: 'm' }, { l: isAr ? 'مستخدمو الدفع الفوري' : 'Instant-pay users', d: D.eUsers, f: 2, suf: 'm' }, { l: isAr ? 'قيمة المدفوعات' : 'Payments value', d: D.eVal, f: 1, suf: ' JOD bn' }, { l: isAr ? 'محافظ الموبايل' : 'Wallet users', d: D.eAtm, f: 2, suf: 'm' }].map(function (x: any, i: number) { return (
