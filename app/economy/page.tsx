...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 14, marginTop: 14 }}>
        ...
        <div style={card}><div style={h2}>{lang === 'ar' ? 'الناتج المحلي حسب النشاط · الربع الأول 2025' : 'GDP BY SECTOR · Q1 2025'}</div><div style={{ marginTop: 8 }}>{(D.gdpSec || []).map(function (s: any, i: number) { return <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '5px 0', fontSize: 12 }}><div style={{ width: '40%', color: 'var(--cf-ink2, #3d4f66)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{lang === 'ar' ? s.ar : s.en}</div><div style={{ flex: 1, background: 'var(--cf-line, #e5eaf2)', borderRadius: 4, height: 13 }}><div style={{ width: Math.round(s.v / 19.5 * 100) + '%', background: 'var(--cf-primary-strong, #0f4c81)', height: 13, borderRadius: 4 }} /></div><div style={{ width: 42, textAlign: 'right', fontWeight: 700, color: 'var(--cf-ink, #0f2a4a)' }}>{s.v}%</div><div style={{ width: 52, textAlign: 'right', fontWeight: 600, fontSize: 11, color: s.g == null ? 'var(--cf-ink3, #7d8ea3)' : (s.g >= 0 ? '#0a7d3c' : '#c0392b') }}>{s.g == null ? '—' : (s.g > 0 ? '+' : '') + s.g + '%'}</div></div> })}</div><div style={{ fontSize: 10.5, color: 'var(--cf-ink3)', marginTop: 6 }}>{lang === 'ar' ? 'دائرة الإحصاءات العامة · مساهمة القطاعات · الربع الأول 2025' : 'Dept. of Statistics · sector share of GDP · Q1 2025'}</div></div>
