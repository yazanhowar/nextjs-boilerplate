...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 14, marginTop: 14 }}>
        ...
        <div style={card}><div style={h2}>{lang === 'ar' ? 'أدوات السياسة النقدية · 2025' : 'CBJ POLICY RATES · 2025'}</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>{(D.polRates || []).map(function (r: any, i: number) { return <div key={i} style={{ border: '1px solid var(--cf-line, #e5eaf2)', borderRadius: 8, padding: '8px 10px' }}><div style={{ fontSize: 10.5, color: 'var(--cf-ink3, #7d8ea3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{lang === 'ar' ? r.ar : r.en}</div><div style={{ fontSize: 18, fontWeight: 800, color: 'var(--cf-ink, #0f2a4a)' }}>{r.v}%</div></div> })}</div><div style={{ fontSize: 10.5, color: 'var(--cf-ink3)', marginTop: 6 }}>{lang === 'ar' ? 'البنك المركزي الأردني · دون تغيير في 2025' : 'CBJ · unchanged through 2025'}</div></div>
